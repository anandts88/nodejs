...
var paths = {
  ...
  certificateFiles: ['./app/bin/cert.csr', './app/bin/certificate.pem', './app/bin/private.key'],
  destination: 'dist'
};
...
// Copy certificate files to dist directory
gulp.task('copy-certificate', function() {
  return _copy(paths.certificateFiles, paths.destination + '/bin');
});
...
gulp.task('nodemon', function() {
	return plugins.nodemon({
		script: path.join('dist', './bin/www'),
    exec: 'node-inspector & node --debug',
		ext: 'js',
		ignore: ['node_modules/**/*.js', 'dist/**/*.js'],
		tasks: ['eslint', 'copy-non-js', 'babel', 'copy-certificate',],
...
gulp.task('serve', function(cb) {
  runSequence('clean', 'eslint', 'copy-non-js', 'babel', 'copy-certificate', 'nodemon', cb);
});
